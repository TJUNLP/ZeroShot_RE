...
def test_model3(nn_model, tag2sentDict_test):
    ...
    while threshold == 0.0:
        ...
        for i in range(len(predictions) // width) :
            ...
            subpredictions = predictions[left:right]
            subpredictions = subpredictions[:, 1].flatten().tolist()
...
def SelectModel(modelname, wordvocabsize, tagvocabsize, posivocabsize,charvocabsize,
    ...
    if modelname is 'Model_ONBiLSTM_directClassifybyLSTM_1':

        nn_model = Model_ONBiLSTM_directClassifybyLSTM_1(wordvocabsize=wordvocabsize,
...
def Dynamic_get_trainSet(istest):
    ...
    train_tag_p = np.asarray(pairs_train[8], dtype="int32")
    train_tag_n = np.asarray(pairs_train[9], dtype="int32")

    # train_y0 = np.zeros(len(labels_train), dtype="int32")
    train_y0 = np.asarray(labels_train0, dtype="int32")
    train_y1 = np.asarray(labels_train1, dtype="int32")
    ...
    inputs_train_x = [train_x1_sent, train_x1_e1_posi, train_x1_e2_posi, train_x1_sent_cahr,
                      ...
                      train_tag_p, train_tag_n]
    inputs_train_y = [train_y1, train_y0, train_y0, train_y1]
...
if __name__ == "__main__":
    ...
    modelname = 'Model_ONBiLSTM_directClassifybyLSTM_1'
