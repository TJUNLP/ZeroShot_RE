...
from NNstruc.NN_Siamese_SentTriplet import Model_BiLSTM_SentPair_tripletloss_nonL_withRank
import keras
...
def get_sent_index(nn_model, inputs_train_x, tagIndex, w2file):
    ...
    intermediate_layer_model_x2 = keras.models.Model(inputs=nn_model.input,
                                                  outputs=nn_model.get_layer('class_BiLSTM').output)
...
def get_sent2vec(nn_model, tag2sentDict_test, w2file):
    ...
    intermediate_layer_model_x2 = keras.models.Model(inputs=nn_model.input,
                                                  outputs=nn_model.get_layer('class_BiLSTM').output)
    intermediate_output_x2 = intermediate_layer_model_x2.predict(inputs_train_x, verbose=1, batch_size=batch_size)
...
def test_model3(nn_model, tag2sentDict_test):
    ...
    inputs_train_x = [train_x1_sent, train_x1_e1_posi, train_x1_e2_posi, train_x1_sent_cahr,
                      train_x2_sent, train_x2_e1_posi, train_x2_e2_posi, train_x2_sent_cahr,
                      train_x3_sent, train_x3_e1_posi, train_x3_e2_posi, train_x3_sent_cahr, train_tag, train_tag]
    ...
    while threshold <= 1.01:
        ...
        for i in range(len(predictions_class) // width) :
            ...
            subpredictions_class = subpredictions_class.flatten().tolist()
        ...
        if F > max_F[2]:
            max_F = [P, R, F]
            max_F_where = threshold
        threshold += 0.2

    return max_F[0], max_F[1], max_F[2], max_F_where
...
def train_e2e_model(nn_model, modelfile, inputs_train_x, inputs_train_y,
    ...
    while nowepoch <= npoches:
        ...
        P, R, F, th = test_model3(nn_model, tagDict_test)
        if F > maxF:
            earlystop = 0
            maxF = F
            nn_model.save_weights(modelfile, overwrite=True)
...
def SelectModel(modelname, wordvocabsize, tagvocabsize, posivocabsize,charvocabsize,
                     word_W, posi_W, tag_W, char_W,rank_W,
                     input_sent_lenth,
                     w2v_k, posi2v_k, tag2v_k, c2v_k, rank2v_k,
                     batch_size=32):
    nn_model = None

    if modelname is 'Model_BiLSTM_SentPair_tripletloss_nonL_withRank':
        nn_model = Model_BiLSTM_SentPair_tripletloss_nonL_withRank(wordvocabsize=wordvocabsize,
                                                  posivocabsize=posivocabsize,
                                                  charvocabsize=charvocabsize,
                                                    tagvocabsize=tagvocabsize,
                                                  word_W=word_W, posi_W=posi_W, char_W=char_W, tag_W=tag_W, rank_W=rank_W,
                                                  input_sent_lenth=input_sent_lenth,
                                                  input_maxword_length=max_c,
                                                  w2v_k=w2v_k, posi2v_k=posi2v_k, c2v_k=c2v_k, tag2v_k=tag2v_k, rank2v_k=rank2v_k,
                                                  batch_size=batch_size)
...
def Dynamic_get_trainSet(istest):
    ...
    pairs_train, labels_train = ProcessData_Siamese_SentPair.CreateTriplet_withMSE(tagDict)
    ...
    inputs_train_x = [train_x1_sent, train_x1_e1_posi, train_x1_e2_posi, train_x1_sent_cahr,
                      train_x2_sent, train_x2_e1_posi, train_x2_e2_posi, train_x2_sent_cahr,
                      train_x3_sent, train_x3_e1_posi, train_x3_e2_posi, train_x3_sent_cahr, train_tag, train_rank]
...
if __name__ == "__main__":
    ...
    modelname = 'Model_BiLSTM_SentPair_tripletloss_nonL_withRank'
    ...
    for inum in range(0, 3):
        ...
        rank2v_k, rank_W = ProcessData_Siamese_SentPair.get_relembed_sim_rank(type_W=type_W)
        ...
        nn_model = SelectModel(modelname,
                               ...
                               word_W=word_W, posi_W=posi_W, tag_W=type_W, char_W=char_W, rank_W=rank_W,
                               input_sent_lenth=max_s,
                               w2v_k=w2v_k, posi2v_k=max_posi + 1, tag2v_k=type_k, c2v_k=c2v_k, rank2v_k=rank2v_k,
                               batch_size=batch_size)
