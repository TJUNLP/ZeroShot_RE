...
def Model_BiLSTM_RankMAP_DyMax_TiedExp_tripletloss_1(wordvocabsize, posivocabsize, charvocabsize, tagvocabsize,
                     word_W, posi_W, char_W, tag_W,
                     input_sent_lenth, input_maxword_length,
                     w2v_k, posi2v_k, c2v_k, tag2v_k,
                    batch_size=32):
    ...
    loss_max = Lambda(lambda x: K.exp(x[2]) * K.relu(0.1 + x[0] - x[1]))\
...
def Model_BiLSTM_RankMAP_DyMax_FocusExp_tripletloss_1(wordvocabsize, posivocabsize, charvocabsize, tagvocabsize,
                     word_W, posi_W, char_W, tag_W,
                     input_sent_lenth, input_maxword_length,
                     w2v_k, posi2v_k, c2v_k, tag2v_k,
                    batch_size=32):
...
def Model_BiLSTM_RankMAP_All_tripletloss_1(wordvocabsize, posivocabsize, charvocabsize, tagvocabsize,
    ...
    # loss = Lambda(lambda x: K.exp(x[2]) * K.relu(0.1 + x[0] - x[1])
    #               )([wrong_cos_1, right_cos, right2i_tag_cos])
    loss = Lambda(lambda x: K.relu(0.1 + x[0] - x[1])
                  )([wrong_cos_1, right_cos, right2i_tag_cos])
    ...
    for i in range(2, tagvocabsize):
        ...
        right2i_tag_cos = Dot(axes=-1, normalize=True)([tag_embedding_p, tag_embedding_n])
        wrong_cos_i = Dot(axes=-1, normalize=True)([BiLSTM_x1, tag_embedding_n])
        loss = Lambda(lambda x: x[3] + K.relu(0.1 + x[0] - x[1])
