...
def test_model4(nn_model, tag2sentDict_test):
    ...
    for tag in tag2sentDict_test.keys():
        ...
        if tag not in tagDict_prototypes.keys():
            continue

        sents = tag2sentDict_test[tag]

        for si, ty in enumerate(tagDict_prototypes.keys()):
            for vi in range(len(tagDict_prototypes[ty])):
                for s in range(1, len(sents)):
                    data_s, data_e1_posi, data_e2_posi, char_s = tagDict_prototypes[ty][vi]
